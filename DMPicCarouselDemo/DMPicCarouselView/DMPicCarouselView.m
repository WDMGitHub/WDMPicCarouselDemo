...
@interface DMPicCarouselView ()<UIScrollViewDelegate>
...
@implementation DMPicCarouselView
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        self = [[NSBundle mainBundle] loadNibNamed:@"DMPicCarouselView" owner:self options:nil].lastObject;
        self.frame = frame;
        oneWidth = frame.size.width;
        [self doWithCreatWithUISize:frame.size];
    }
    return self;
}

- (void)doWithCreatWithUISize:(CGSize)size {
//    self.scrollView.pagingEnabled = YES;
    self.scrollView.contentSize = CGSizeMake(3 * size.width, size.height);
    self.scrollView.delegate = self;
    self.preImgView.frame = CGRectMake(0, 0, size.width, size.height);
    self.currentImgView.frame = CGRectMake(size.width, 0, size.width, size.height);
    self.nextImgView.frame = CGRectMake(2 * size.width, 0, size.width, size.height);
    //偏移量一直在currentImgView的位置
...
- (void)doWithImgView {
    ...
    if (!self.isNetPic) {
        ...
        //网络请求
//        [self.preImgView sd_setImageWithURL:[NSURL URLWithString:[self.picModels[preIndex] picUrl]] placeholderImage:[UIImage imageNamed:@""]];
//        [self.currentImgView sd_setImageWithURL:[NSURL URLWithString:[self.picModels[currentIndex] picUrl]] placeholderImage:[UIImage imageNamed:@""]];
//        [self.nextImgView sd_setImageWithURL:[NSURL URLWithString:[self.picModels[nextIndex] picUrl]] placeholderImage:[UIImage imageNamed:@""]];
